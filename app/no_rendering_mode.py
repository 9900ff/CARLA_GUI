...
class World(object):
    ...
    def select_hero_actor(self):
        """
        Selects an actor based on the role name provided in the arguments.
        It no longer spawns a new one if not found.
        """
        # ### 关键修改：使用 self.args.role_name 来过滤 ###
        # 从世界中寻找所有类型为车辆且角色名与我们指定参数相符的actor
        hero_vehicles = [actor for actor in self.world.get_actors()
                         if 'vehicle' in actor.type_id and actor.attributes['role_name'] == self.args.role_name]
        ...
        if len(hero_vehicles) > 0:
            ...
            # 确保我们不会意外地控制它
            self.hero_actor.set_autopilot(False)
            self._hud.notification(f"Now following actor with role_name='{self.args.role_name}'", seconds=4.0)
        else:
            # ### 关键修改：如果没找到，不再调用 _spawn_hero() ###
            # 而是将 hero_actor 设为 None，并打印提示信息
            self.hero_actor = None
            print(f"Waiting for actor with role_name='{self.args.role_name}' to spawn...")
...
def main():
    ...
    argparser.add_argument(
        '--role-name',
        metavar='NAME',
        default='ego_car', # 默认值设为 'ego_car'
        help='role name of the actor to follow (default: "hero")')
    argparser.add_argument(
        '--map',
        metavar='TOWN',
        default=None,
