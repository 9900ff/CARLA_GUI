...
def game_loop(args):
    ...
    try:
        ...
        prev_sync_mode = sim_world.get_settings().synchronous_mode  # 初始世界模式
        if prev_sync_mode == True:
            print("[提示] CARLA 当前为同步模式")
        else:
            print("[提示] CARLA 当前为异步模式")

        while True:
            # 判断世界模式
            current_sync_mode = sim_world.get_settings().synchronous_mode # 当前世界模式
            if current_sync_mode != prev_sync_mode: # 世界模式发生改变
                if current_sync_mode == True:
                    print("[提示] CARLA 切换为同步模式")
                else:
                    print("[提示] CARLA 切换为异步模式")
                prev_sync_mode = current_sync_mode  # 更新旧值

            # clock.tick_busy_loop(60)
            clock.tick(60) # 该刷新方法不会导致同步模式延迟
...
def main():
    ...
    argparser.add_argument(
        '--sync',
        action='store_true',
        # default='True', # 保留这句为同步模式，注释掉为异步模式
